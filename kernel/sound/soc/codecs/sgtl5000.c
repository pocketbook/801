...
///kernel_imx_imx508/sound/soc/codecs
...
///mode define
///capless headphone
#define SGTL5000_CAPLESS_HEADPHONE_MODE 
//#define SGTL5000_NO_CAPLESS_HEADPHONE_MODE
...
extern int get_pcm_audio_streamstate(void);
...
static void sgtl5000_clock_gating(struct snd_soc_codec *codec, int enable)
{
	...
	if (enable == 0) {
		...
		cancel_delayed_work(&codec->delayed_work);/* add from patch*/
		if (!sgtl5000->clock_on) {
			sgtl5000->clock_enable(1);
			sgtl5000->clock_on = 1;
			msleep(1);/* add from patch*/
		}
	}
}
...
{
	//#if defined(SGTL5000_CAPLESS_HEADPHONE_MODE)	
	if(reg == SGTL5000_CHIP_ANA_HP_CTRL){
	   // minus 0x18 to HP_VOL_RIGHT and HP_VOL_LEFT 
	   //after SGTL5000_CHIP_ANA_HP_CTRL is read
	   int regvalue =0;
	   regvalue = sgtl5000_read_reg_cache(codec, reg);
	   regvalue -=SGTL5000_HP_VOL_COMPENDATION_VALUE2;
	   return regvalue;
	}//else if (reg == SGTL5000_CHIP_ANA_ADC_CTRL)
	//{
	//   int regADCvalue =0;
	//   regADCvalue = sgtl5000_read_reg_cache(codec, reg);
	//   regADCvalue |=SGTL5000_ADC_VOL_COMPENDATION_VALUE;
	//   return regADCvalue;
	//}
	else if ((reg == SGTL5000_CHIP_ID) ||
...
//#elif defined(SGTL5000_NO_CAPLESS_HEADPHONE_MODE)
	//if ((reg == SGTL5000_CHIP_ID) ||
	//    (reg == SGTL5000_CHIP_ADCDAC_CTRL) ||
	//    (reg == SGTL5000_CHIP_ANA_STATUS) ||
	//    (reg > SGTL5000_MAX_CACHED_REG)){
	//	return sgtl5000_hw_read(codec, reg);
	//}else if(reg == SGTL5000_CHIP_ANA_HP_CTRL){
	//    int regvalue =0;
	//     regvalue = sgtl5000_read_reg_cache(codec, reg);
	//   return regvalue;
	//}else{
			  ...
			  unsigned int value)
{
	...
	sgtl5000_clock_gating(codec, 1);
//#if defined(SGTL5000_CAPLESS_HEADPHONE_MODE)
       ////add 0x18 to HP_VOL_RIGHT and HP_VOL_LEFT 
       ///when  writing SGTL5000_CHIP_ANA_HP_CTRL 
       if(reg == SGTL5000_CHIP_ANA_HP_CTRL){
	   	value +=SGTL5000_HP_VOL_COMPENDATION_VALUE2;
	   	sgtl5000_write_reg_cache(codec, reg, value);
       	//}else if(reg == SGTL5000_CHIP_ANA_ADC_CTRL)
       	//{
   	//      value &=SGTL5000_ADC_VOL_COMPENDATION_VALUE;
   	//      sgtl5000_write_reg_cache(codec, reg, value);
       	}else{
       	     sgtl5000_write_reg_cache(codec, reg, value);
       	}     
			 ...
			 struct snd_ctl_elem_value *ucontrol)
{
	...
	/*default reg is 0x3c3c*/


	//reg |= SGTL5000_DAC_VOL_LEFT_NEGATIVE_5DB |
	//	    SGTL5000_DAC_VOL_RIGHT_NEGATIVE_5DB;
/*
        ///minus the DAC vol level,now don't minus it.
	reg &= SGTL5000_DAC_VOL_LEFT_NEGATIVE_20DB;

	reg &=SGTL5000_DAC_VOL_RIGHT_NEGATIVE_20DB;


	reg |= SGTL5000_DAC_VOL_LEFT_NEGATIVE_20DB |
		SGTL5000_DAC_VOL_RIGHT_NEGATIVE_20DB;

*/
	sgtl5000_write(codec, SGTL5000_CHIP_DAC_VOL, reg);

	//sgtl5000_write(codec, SGTL5000_CHIP_DAC_VOL, 0x4040);
...
#if defined(SGTL5000_NO_CAPLESS_HEADPHONE_MODE)	 
	SOC_DOUBLE("Headphone Volume", SGTL5000_CHIP_ANA_HP_CTRL, 0, 8, 0x7f,
		   1),
#elif defined(SGTL5000_CAPLESS_HEADPHONE_MODE)
	SOC_DOUBLE("Headphone Volume", SGTL5000_CHIP_ANA_HP_CTRL, 0, 8,0x67,
		   1),
#endif
};
				...
				struct snd_soc_dai *dai)
{
	...
	reg2 = reg = sgtl5000_read(codec, SGTL5000_CHIP_DIG_POWER);
	...
	if (substream->stream == SNDRV_PCM_STREAM_CAPTURE) {
		reg2 = reg = sgtl5000_read(codec, SGTL5000_CHIP_ANA_POWER);
...
#endif
		//reg |= SGTL5000_CAPLESS_HEADPHONE_MOED;
		sgtl5000_write(codec, SGTL5000_CHIP_ANA_POWER, reg);
		msleep(400);
...
static int sgtl5000_probe(struct platform_device *pdev)
{
	...
	lo_ctrl |=
        ...
        /*wen add,changes the register(0x0026) value:0x28,but this modify make
		the power consumption increase 10 mA(need change hardware)*/
 	//lreg_ctrl |= SGTL5000_VDDC_ASSN_OVRD;
	/* set short detect */
	/* keep default */
	...
	sgtl5000_write(codec, SGTL5000_CHIP_MIC_CTRL, 0);
       // sgtl5000_write(codec, SGTL5000_CHIP_MIC_CTRL, 0x170);
	sgtl5000_write(codec, SGTL5000_CHIP_CLK_TOP_CTRL, 0);
       /* for sound control it is to big */
       sgtl5000_write(codec, SGTL5000_CHIP_DAC_VOL, 0x3d3d);
       /*write the CHIP_ANA_HP_CTRL register ,set the hp vol,wen add,*/
	//  sgtl5000_write(codec, SGTL5000_CHIP_ANA_HP_CTRL, 0x1818);
	/* disable DAP */
	sgtl5000_write(codec, SGTL5000_DAP_CTRL, 0);

      /*wen add.for line out volume test*/
	reg = sgtl5000_read(codec, SGTL5000_CHIP_LINE_OUT_VOL);
        reg&=0x0;
	reg|=0xb0b;
	//printk("%s,%s,%d (0x002e)reg =0x%x  \n",__FILE__, __func__, __LINE__,reg);
	sgtl5000_write(codec, SGTL5000_CHIP_LINE_OUT_VOL, reg);
	...
	sgtl5000_add_widgets(codec);
         ...
         ///in defaul,we enter the off state.  test ..
	sgtl5000_set_bias_level(codec, SND_SOC_BIAS_STANDBY);
	//sgtl5000_set_bias_level(codec, SND_SOC_BIAS_OFF);
					...
					const struct i2c_device_id *id)
{
	...
	//pr_info("r r:%02x,v:%04x bylinfortest!\n", SGTL5000_CHIP_ID, val);
...
#elif defined(SGTL5000_CAPLESS_HEADPHONE_MODE)
        codec->read = sgtl5000_read_reg_cache  ;/*sgtl5000_read ;*/
#endif
	...
	//sgtl5000_write(codec, SGTL5000_CHIP_ANA_HP_CTRL, SGTL5000_HP_VOL_COMPENDATION_VALUE2);/*wen add .*/
...
extern int mx50_rdp_mute_switch(bool bMute);
static __devexit int sgtl5000_i2c_shutdown(struct i2c_client *client)
{
	...
	u16 adcdac_ctrl;
        ...
        ///we mute the sgtl5000 before shutdown
	adcdac_ctrl = sgtl5000_read(codec, SGTL5000_CHIP_ADCDAC_CTRL);
...
static struct i2c_driver sgtl5000_i2c_driver = {
	...
	//.suspend =sgtl5000_i2c_suspend,
	//.resume =sgtl5000_i2c_resume,
