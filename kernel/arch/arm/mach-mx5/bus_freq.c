...
extern void dvfs_load_config(int set_point);
...
static void mxc_busfreq_early_suspend(struct early_suspend *h)
{
	if (low_bus_freq_mode)
		set_high_bus_freq(1);
	busfreq_suspended = 1;
...
static void mxc_busfreq_late_resume(struct early_suspend *h)
{
	busfreq_suspended = 0;
...
static struct early_suspend mxc_busfreq_earlysuspend = {
	.level = EARLY_SUSPEND_LEVEL_BLANK_SCREEN,
	.suspend = mxc_busfreq_early_suspend,
	.resume = mxc_busfreq_late_resume,
};
#endif
...
#ifdef CONFIG_EARLYSUSPEND
	register_early_suspend(&mxc_busfreq_earlysuspend);
#endif
...
#ifdef CONFIG_EARLYSUSPEND
	unregister_early_suspend(&mxc_busfreq_earlysuspend);
#endif
