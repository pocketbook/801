...
static void usbh1_set_utmi_xcvr(void)
{
	...
	if (cpu_is_mx50()) {
		USBCTRL |= UCTRL_H1PM; /* Host1 Power Mask */
		 ...
		 /* wen modify. .2012-02-25,for reduce the 20mA power consumption.
		   *  we found one question:  the power consume 
	           *  increased 20MA  when I changed the OWIRE from USBHI_OC mode 
	           * to SSI_EXT1_CLK mode.
	           */
...
static void otg_set_utmi_xcvr(void)
{
	...
	if (cpu_is_mx51()) {
		...
		/* wen  Disabled  OTG Overcurrent Event */
		USB_PHY_CTR_FUNC |= USB_UTMI_PHYCTRL_OC_DIS;
	} else {
		/* USBOTG_PWR low active */
		USBCTRL &= ~UCTRL_PP;
