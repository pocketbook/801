...
void stop_dvfs(void);
void dvfs_load_config(int set_point);
...
static int set_cpu_freq(int wp)
{
	...
	if (cpu_wp_tbl[wp].pll_rate != cpu_wp_tbl[old_wp].pll_rate) {
		...
		arm_podf = reg & 0x07;
		if (podf == arm_podf) {
			printk(KERN_DEBUG"No need to change freq and voltage!!!!\n");
			return 0;
		}
...
static void mxc_dvfs_early_suspend(struct early_suspend *h)
{
	if (dvfs_core_is_active) {
		dvfs_core_resume = 1;
		stop_dvfs();
...
static void mxc_dvfs_late_resume(struct early_suspend *h)
{
	if (dvfs_core_resume) {
		dvfs_core_resume = 0;
		start_dvfs();
...
static struct early_suspend mxc_dvfs_earlysuspend = {
	//.level = EARLY_SUSPEND_LEVEL_DISABLE_FB,
	.level = EARLY_SUSPEND_LEVEL_BLANK_SCREEN,
	.suspend = mxc_dvfs_early_suspend,
	.resume = mxc_dvfs_late_resume,
};
#endif
...
#ifdef CONFIG_EARLYSUSPEND
	register_early_suspend(&mxc_dvfs_earlysuspend);
#endif
...
#ifdef CONFIG_EARLYSUSPEND
	unregister_early_suspend(&mxc_dvfs_earlysuspend);
#endif
