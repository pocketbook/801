...
int pwm_config(struct pwm_device *pwm, int duty_ns, int period_ns)
{
	...
	if (!(cpu_is_mx1() || cpu_is_mx21())) {
		...
		//	c = clk_get_rate(pwm->clk);//8000000
		...
		writel(0, pwm->mmio_base + 0x08); 
		writel(duty_cycles, pwm->mmio_base + MX3_PWMSAR);
		period_reg = readl(pwm->mmio_base + MX3_PWMPR);
		if (period_reg != period_cycles) {
			//printk("<4>Setting period reg old x%08x new x%08lx\n", period_reg, period_cycles);
		...
		cr_reg = readl(pwm->mmio_base + MX3_PWMCR);
		if ((cr_reg  & ~MX3_PWMCR_EN) != cr) {
			// do not touch enable
			cr |= (cr_reg & MX3_PWMCR_EN);
			writel(cr, pwm->mmio_base + MX3_PWMCR);
			//printk("<4>Setting cr old x%08x new x%08x\n", cr_reg, cr);
		}
...
int pwm_clock_enabled(struct pwm_device *pwm)
{
	return pwm->clk_enabled; /* check if clock enabled */
}
EXPORT_SYMBOL(pwm_clock_enabled);
