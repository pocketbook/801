...
static irqreturn_t mxc_usb_wakeup_handler(int irq, void *_dev)
{
	struct wakeup_ctrl *ctrl = (struct wakeup_ctrl *)_dev;
	irqreturn_t ret = IRQ_NONE;

	if (usb2_is_in_lowpower(ctrl)) {
		printk(KERN_INFO "%s %s %d:  usb wakeup is here\n",__FILE__,__func__,__LINE__);
...
static enum usb_wakeup_event mxc_is_wakeup(struct fsl_usb2_platform_data *pdata)
{
...
static void mxc_wakeup_event_handler(struct wakeup_ctrl *ctrl)
{
	struct fsl_usb2_wakeup_platform_data *pdata = ctrl->pdata;
	int already_waked = 0;
	...
	for (i = 0; i < 3; i++) {
		...
		if (usb_pdata) {
			...
			wakeup_evt = mxc_is_wakeup(usb_pdata);
...
static int mxc_wakeup_event_thread(void *param)
{
...
static int mxc_wakeup_dev_probe(struct platform_device *pdev)
{
	struct fsl_usb2_wakeup_platform_data *pdata;
	...
	status = request_irq(ctrl->wakeup_irq, mxc_usb_wakeup_handler,IRQF_SHARED, "usb_wakeup", (void *)ctrl);
	if (status){
		goto error1;
	}
	ctrl->usb_irq = platform_get_irq(pdev, 1);

	ctrl->thread = kthread_run(mxc_wakeup_event_thread, (void *)ctrl, "usb_wakeup thread");
...
static int  mxc_wakeup_dev_exit(struct platform_device *pdev)
{
	if (g_ctrl->thread) {
...
static struct platform_driver wakeup_d = {
	.probe   = mxc_wakeup_dev_probe,
	.remove  = mxc_wakeup_dev_exit,
	.driver = {
		.name = "usb_wakeup",
	},
};
