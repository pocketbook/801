...
#ifdef CONFIG_CCX
	u8 *ccx_version_ie; /* CCX Version IE */
	size_t ccx_version_ie_len;
	u8 *cckm_req_ie; /* CCKM Request IE */
	size_t cckm_req_ie_len;
	u8 *cckm_resp_ie; /* CCKM Response IE */
	size_t cckm_resp_ie_len;
	u8 *connect_wpa_ie;
	size_t connect_wpa_ie_len;
#endif
...
#ifdef CONFIG_CCX
static inline int wpa_sm_update_cckm_ies(struct wpa_sm *sm,
					const u8 *ies, size_t ies_len)
{
	return -1;
}
