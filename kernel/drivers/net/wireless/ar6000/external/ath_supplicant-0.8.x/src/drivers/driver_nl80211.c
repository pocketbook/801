...
#ifdef CONFIG_WAPI
		case WPA_ALG_SMS4:
			/*in here, we ONLY borrow tkip for WAPI*/
			NLA_PUT_U32(msg, NL80211_ATTR_KEY_CIPHER,
				    WLAN_CIPHER_SUITE_TKIP);
			break;
#endif
...
#ifdef CONFIG_WAPI
    /*special note: in nl80211, the seq_len needs 6,
      otherwise it will check fail.
      we will set the wapi seq_len in our driver again.
     */
    if (alg == WPA_ALG_SMS4)
    seq_len = 6;
#endif
...
#ifdef CONFIG_WAPI
        /*in wapi, we must send this mac address*/
		if (alg == WPA_ALG_SMS4)
		NLA_PUT(msg, NL80211_ATTR_MAC, ETH_ALEN, addr);
#endif		
		types = nlmsg_alloc();
...
#ifdef CONFIG_WAPI
		else if(params->pairwise_suite == CIPHER_SMS4)
			ver = 0;
#endif
		else
