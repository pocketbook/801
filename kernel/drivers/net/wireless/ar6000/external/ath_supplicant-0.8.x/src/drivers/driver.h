...
#ifdef ANDROID
#define WPAS_MAX_SCAN_SSIDS 9 /* for combo scan */
#else
...
struct wpa_driver_ops {
    ...
    /**
     * p2p_capa_init - Initialize driver P2P capability
     */
	...
	/**
	 * p2p_cancel - Cancel Pending P2P Operations
	 * @priv: Private driver interface data
	 * Returns: 0 on success, -1 on failure
	 *
	 * This function is only used if the driver implements P2P management,
	...
	/**
	 * p2p_flush - Stop p2p state machine (halt any connection process
	 * if one is in progress) and clear the list of p2p peers.
	 * @priv: Private driver interface data
	 *
	 * This function is only used if the driver implements P2P management,
	 * i.e., if it sets WPA_DRIVER_FLAGS_P2P_MGMT in
	...
	/**
	 * p2p_auth_go_neg - Authorize group formation (GO negotiation) with
	 * specified peer.
	 */
	...
	int (*signal_poll)(void *priv, struct wpa_signal_info *signal_info);
	   /**
     * p2p_group_init - Initializes a P2P grounp being formed.
     */ 
    ...
    int (*p2p_auth_invite)(void *priv, int auth, const u8 *auth_peer);
	 ...
	 *
	 */
    int (*driver_cmd)(void *priv, const char *cmd, char *buf, size_t buf_len);
...
#ifdef CONFIG_CCX
	/**
	 * EVENT_CHANNEL - Report Current Operating Channel
	 *
	 * The driver is expected to report the current operating channel
	 * on association with a new access point. This is used by CCX
	 * module for AP reporting
	 */
	EVENT_CHANNEL,
#endif
};
