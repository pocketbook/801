...
void eap_notify_refresh_timer(struct eap_sm *sm)
{
	if (sm) {
               wpa_printf(MSG_DEBUG, "EAP: External notification - "
                   "EAP portTimer refresh %d=>%d", sm->ClientTimeout, EAP_CLIENT_TIMEOUT_DEFAULT);
                sm->ClientTimeout = EAP_CLIENT_TIMEOUT_DEFAULT;
                eapol_set_int(sm, EAPOL_idleWhile, sm->ClientTimeout);
	}
}
