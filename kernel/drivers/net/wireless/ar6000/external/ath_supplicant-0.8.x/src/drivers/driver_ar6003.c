...
#ifdef ANDROID
    drv->capa.max_scan_ssids = (WEXT_CSCAN_AMOUNT < MAX_PROBED_SSID_INDEX) ? WEXT_CSCAN_AMOUNT : MAX_PROBED_SSID_INDEX;
#else
...
#endif /* CONFIG_P2P */
#ifdef CONFIG_CCX
	} else if (os_strncmp(custom, "ASSOCRESPIE=", 12) == 0) {
		char *spos;
		int bytes;
		u8 *resp_ies = NULL;
...
#ifdef CONFIG_CCX
static void wext_get_channel(struct iw_event *iwe, u32 *channel)
{
	int divi = 1000000, i;
	int freq = 0;
	...
	if (iwe->u.freq.e > 6) {
		wpa_printf(MSG_DEBUG, "Invalid freq m=%d e=%d)",
                   ...
                   char *data, int len)
{
    ...
    switch (iwe->cmd) {
		...
		case SIOCGIWFREQ:
			{
			union wpa_event_data data;
			wext_get_channel(iwe, &data.channel.channel);
			wpa_printf(MSG_INFO, "Wireless event: Channel %d",
					data.channel.channel);
			wpa_supplicant_event(drv->ctx, EVENT_CHANNEL, &data);
			break;
			}
#endif
    case SIOCGIWAP:
 ...
 */
int ar6003_driver_combo_scan(void *priv, struct wpa_driver_scan_params *params)
{
	char buf[WEXT_CSCAN_BUF_LEN];
	struct ar6003_driver_data *drv = (struct ar6003_driver_data *)priv;
	struct iwreq iwr;
	int ret = 0, i = 0, bp;
	const u8 *ssid_nm = NULL;
	size_t ssid_len = 0;
	...
	for (i=0; i<(int)params->num_ssids && i < drv->capa.max_scan_ssids; ++i) {
		if ((bp + IW_ESSID_MAX_SIZE + 10) >= (int)sizeof(buf))
			break;
		ssid_nm = params->ssids[i].ssid;
		ssid_len = params->ssids[i].ssid_len;
		if (ssid_len > 0) {
			wpa_printf(MSG_DEBUG, "For Scan: %s", ssid_nm);
			buf[bp++] = WEXT_CSCAN_SSID_SECTION;
			buf[bp++] = ssid_len;
			os_memcpy(&buf[bp], ssid_nm, ssid_len);
			bp += ssid_len;
		}
	}
	...
	if (!params->freqs) {
		...
		for (i=0; params->freqs[i] != 0; i++) {
			buf[bp++] = params->freqs[i];
			if (params->freqs[i+1]!=0) {
				buf[bp++] = WEXT_CSCAN_CHANNEL_SECTION;
			}
		}
	}
...
int ar6003_driver_scan(void *priv, struct wpa_driver_scan_params *params)
{
    ...
    if (params->num_ssids > 1) {
        ret = ar6003_driver_combo_scan(priv, params);
    } else {
#endif /* ANDROID */
...
#ifdef CONFIG_WAPI
    /* Clear APP IE */
    ar6003_driver_set_gen_ie(drv, (const u8 *)&temp, 0);
#endif
...
#ifdef CONFIG_WAPI
	    || params->pairwise_suite == CIPHER_SMS4
#endif
    )
...
static int ar6003_driver_p2p_auth_invite(void *priv, int auth, const u8 *peer_addr)
{
    struct ar6003_driver_data *drv = priv;
    char buf[256];
    struct ifreq ifr;
    P2P_AUTH_INVITE_CMD *cmd;
    ...
    ((int *)buf)[0] = AR6000_XIOCTL_P2P_AUTH_INVITE;
    cmd = (P2P_AUTH_INVITE_CMD *)(&(((int *)buf)[1]));
    cmd->auth = auth; /* Auth or unauth */
    os_memcpy(cmd->peer_addr, peer_addr, 6); /* The peer address to perform operation on */
...
static int ar6003_driver_p2p_set_config(void *priv, char *cmd)
{
    ...
    *param++ = '\0';
    ...
    } else if (os_strcmp(cmd, "go_intent") == 0) {
        config->config_id = WMI_P2P_CONFID_GO_INTENT;
        config->val.go_intent.value  = atoi(param);
    } else if (os_strcmp(cmd, "dev_name") == 0) {
        config->config_id = WMI_P2P_CONFID_DEV_NAME;
        config->val.device_name.dev_name_len = os_strlen(param);
        A_MEMCPY(config->val.device_name.dev_name, param, os_strlen(param));
