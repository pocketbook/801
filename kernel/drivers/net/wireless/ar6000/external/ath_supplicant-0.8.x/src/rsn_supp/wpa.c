			       ...
			       struct wpa_ptk *ptk)
{
	...
	if (wpa_key_mgmt_cckm(sm->key_mgmt))
		wpa_eapol_key_send(sm, sm->cgk.krk, ver, dst, ETH_P_EAPOL,
...
#ifdef CONFIG_CCX
	if (wpa_key_mgmt_cckm(sm->key_mgmt)) {
		/* Get CCKM Generic Keys KRK+BTK*/
		wpa_nsk_to_cgk(sm->pmk, sm->pmk_len, sm->bssid,
				sm->own_addr, sm->snonce, key->key_nonce,
				(u8 *)&sm->cgk, sizeof(sm->cgk));

		/* Get PTK from BTK */
		wpa_btk_to_ptk(sm->cgk.btk, sizeof(sm->cgk.btk), sm->bssid,
				sm->cckm_rn, (u8 *)ptk, ptk_len);
...
#ifdef CONFIG_CCX
static int wpa_supplicant_install_krk(struct wpa_sm *sm,
		const u8 *krk, int krk_len)
{
	wpa_hexdump_key(MSG_DEBUG, "CCKM: set KRK to the driver", krk, krk_len);

	if (wpa_sm_set_key(sm, WPA_ALG_KRK,
				(u8 *) "\xff\xff\xff\xff\xff\xff",
				0, 0, NULL, 0,
				krk, krk_len) < 0) {
		wpa_dbg(sm->ctx->msg_ctx, MSG_WARNING,
				"CCKM: Failed to set KRK to the driver.");
		return -1;
	}
...
int wpa_sm_set_connect_wpa_ie(struct wpa_sm *sm, const u8 *ie, size_t len)
{
	if (sm == NULL)
		return -1;

	os_free(sm->connect_wpa_ie);
	if (ie == NULL || len == 0) {
		wpa_dbg(sm->ctx->msg_ctx, MSG_DEBUG,
				"WPA: clearing own WPA/RSN IE");
		sm->connect_wpa_ie = NULL;
		sm->connect_wpa_ie_len = 0;
	} else {
		wpa_hexdump(MSG_DEBUG, "WPA: set own WPA/RSN IE", ie, len);
		sm->connect_wpa_ie = os_malloc(len);
		if (sm->connect_wpa_ie == NULL)
			return -1;

		os_memcpy(sm->connect_wpa_ie, ie, len);
		sm->connect_wpa_ie_len = len;
	}
...
#ifdef CONFIG_CCX
	wpa_supplicant_install_krk(sm, sm->cgk.krk, CCKM_KRK_LEN);
#endif
	...
	if ((ie->wpa_ie && sm->ap_wpa_ie &&
	    ...
	    (ie->rsn_ie && sm->ap_rsn_ie &&
	     wpa_compare_rsn_ie(wpa_key_mgmt_ft(sm->key_mgmt),
				sm->ap_rsn_ie, sm->ap_rsn_ie_len,
				ie->rsn_ie, ie->rsn_ie_len))) {
#ifdef STRICT_CHECKING  /* EV# 77486 */
		wpa_report_ie_mismatch(sm, "IE in 3/4 msg does not match "
...
#else  /* EV# 77486 */
                /*
 *          * Some APs(Atheros AR5KAP-0096WFA 7.3.0.385) send different
 *                   * IE in its beacons Skip this error in the 4 way handshake
 *                            * even if IE in 3/4 msg does not match with previous IE in
 *                                     * Beacon/ProbeResp
 *                                              */
                wpa_printf(MSG_WARNING, "IE in 3/4 msg does not match "
                                   "with IE in Beacon/ProbeResp. "
