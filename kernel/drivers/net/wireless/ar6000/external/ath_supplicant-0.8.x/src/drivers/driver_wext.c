...
/* This makes the GUI not working in android */
#ifndef ANDROID
...
#ifndef ANDROID
/*in linux platform, connect with random ssid ap, it will cause WAPI issue.
 *root reason: in CL759252, ar->arWapiEnable will be set to 0 when recving
 *disconnect event.
 */
#if 0
		for (i = 0; i < 32; i++)
...
static int wpa_driver_priv_driver_cmd( void *priv, const char *cmd, char *buf, size_t buf_len )
{
	struct wpa_driver_wext_data *drv = priv;
	struct wpa_supplicant *wpa_s = (struct wpa_supplicant *)(drv->ctx);
	struct iwreq iwr;
	...
	if (os_strcasecmp(cmd, "RSSI-APPROX") == 0) {
		...
		cmd = cmdbuf;
	 } else if (os_strcasecmp(cmd, "SCAN-CHANNELS")==0) {
