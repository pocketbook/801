...
#include "include/ctrl_interface.h"
#include "../../../src/common/defs.h" // for wpa_states
...
static int check_wai_frame(struct wapi_asue_st *wpa_s, const unsigned char *buf, size_t len)
{
	...
	if(len < sizeof(struct wai_hdr)){
		iwn_wpa_printf(MSG_DEBUG, "WPA: WAI frame too short, len %d",  len);
		return -1;
	}
	version = ((hdr->version[0]<<8)| hdr->version[1]);
	GETSHORT(hdr->version, version);
	if( version != WAI_VERSION) {
		iwn_wpa_printf(MSG_DEBUG, "WPA: WAI frame Version(%u) is wrong", version);
		return -1;
	}

	if(hdr->type != WAI_TYPE){
		iwn_wpa_printf(MSG_DEBUG, "WPA: WAI frame type(%u) is wrong", hdr->type);
		return -1;
	}
	
	if(((wpa_s->ap_type == AUTH_TYPE_WAPI_PSK) && (hdr->stype < WAI_USK_NEGOTIATION_REQUEST))
		|| ((hdr->stype < WAI_AUTHACTIVE))){
		iwn_wpa_printf(MSG_DEBUG, "WPA: WAI frame stype(%u) is wrong ",hdr->stype);
		return -1;
	}
	
	GETSHORT((buf+6), frmlen);
	if(len != frmlen){
		iwn_wpa_printf(MSG_DEBUG, "WPA: WAI frame length(%u) is wrong",  frmlen);
		return -1;
	}

	GETSHORT(hdr->rxseq, rxseq);
	if(rxseq < wpa_s->rxseq){
		iwn_wpa_printf(MSG_DEBUG, "WPA: WAI frame packets_sc(%u) is wrong",  rxseq);
		return -1;
	}
	return 0;
}
...
#define MACSTR "%02x:%02x:%02x:%02x:%02x:%02x"
	if(memcmp(wpa_s->bssid, addid, ETH_ALEN) || 
		...
		iwn_wpa_hexdump(MSG_ERROR, "addid", addid ,2*ETH_ALEN);
		iwn_wpa_printf(MSG_ERROR, "bssid="MACSTR"; ownaddr"MACSTR"",
...
{
	const u16 remote_val_type = 0;
	const u16 remote_val_len = 0;
	const u16 local_val_type = 0;
	const u16 local_val_len = 0;
	const u8 *p = NULL;

	p = (const u8 *)remote;
	/*  identifier */
	memcpy((void *)&remote_val_type, (void *)p, sizeof(u16));
	/* data length */
	memcpy((void *)&remote_val_len, (void *)(p+sizeof(u16)), sizeof(u16));

	p = (const u8 *)local;
	/*  identifier */
	memcpy((void *)&local_val_type, (void *)p, sizeof(u16));
	/* data length */
	memcpy((void *)&local_val_len, (void *)(p+sizeof(u16)), sizeof(u16));

	if (iwn_ntohs(remote_val_type) != local_val_type
	     || iwn_ntohs(remote_val_len) != local_val_len
	     || memcmp((char*)remote+4, (char*)local+4, len))
	{
		return -1;
	}
...
static int waigroup_unicast_1_3(struct wapi_asue_st* wpa_s, u8* payload, int len)
{
    ...
    wpa_printf(MSG_DEBUG, "WPA: in %s:%d", __func__, __LINE__);

    if(len < usk_request_len)
    {
        wpa_printf(MSG_ERROR, "WPA: WAI frame payload too short, len(%u)", len);
        return -1;
    }
    flag = payload[0];
    bkid = payload+1;
    ...
    if (memcmp(bkid, wapi_sm->bksa.bkid, BKID_LEN) != 0) {
        wpa_printf(MSG_DEBUG, "WPA: BKID is wrong");
        wpa_hexdump(MSG_ERROR, "AE BKID", bkid, BKID_LEN);
        wpa_hexdump(MSG_ERROR, "ASUE BKID", wapi_sm->bksa.bkid, BKID_LEN);
        wpa_hexdump(MSG_ERROR, "ASUE BK", wapi_sm->bksa.bk, BKID_LEN);
        return -2;
    }

    if((flag & BIT(4)) && (uskid != !wapi_sm->usksa.uskid))
    {
        wpa_printf(MSG_ERROR, "WPA: USKID  is not invalid");
        return -1;
    }

    if((flag & BIT(4)) && check_challege(wapi_sm->usksa.usk[wapi_sm->usksa.uskid].ae_challenge, ae_challenge))
    {
        wpa_hexdump(MSG_ERROR, "ae_challenge", ae_challenge, CHALLENGE_LEN);
        return -1;
    ...
    if(text == NULL)
    {
        wpa_printf(MSG_DEBUG, "WPA: malloc failure");
        return -1;
    }
    pos = text;
    ...
    wpa_hexdump(MSG_DEBUG, "asue_challenge", wapi_sm->usksa.usk[uskid].asue_challenge, 32);
    wpa_hexdump(MSG_DEBUG, "ae_challenge", ae_challenge, 32);
...
static int waigroup_unicast_2_3_send(struct wapi_asue_st* wpa_s, u8* payload, int len_in)
{
	...
	iwn_wpa_printf(MSG_DEBUG, "WPA: in %s:%d\n", __func__, __LINE__);
	...
	if(tbuf == NULL)
		return -1;
	/*iwn_wpa_hexdump(MSG_DEBUG, "WPA: tbuf", tbuf, tbuflen);*/
	pos = tbuf;
	pos = wpa_build_hdr(pos, ++wpa_s->txseq, WAI_USK_NEGOTIATION_RESPONSE);
...
static int waigroup_unicast_3_3(struct wapi_asue_st* wpa_s, u8* payload, int len)
{
    ...
    hmac_sha256(payload, len-WAI_MIC_LEN, usk->mak,PAIRKEY_LEN,mic, WAI_MIC_LEN);
    ...
    /* not unicast update  */
    if(!(payload[0] & BIT(4))){
        if(wapi_sm->ap_wapi_ie_len == 0)//for hidden AP
        {
            wapi_sm->ap_wapi_ie_len = wie_len;
            memcpy(wapi_sm->ap_wapi_ie,wie,wie_len);
        }
        if((wie_len != wapi_sm->ap_wapi_ie_len)||
            memcmp(wie, wapi_sm->ap_wapi_ie, wapi_sm->ap_wapi_ie_len))
        {
            iwn_wpa_hexdump(MSG_ERROR, "AE's IE ", wie, wie_len);
            iwn_wpa_hexdump(MSG_ERROR, "ASUE's IE ", wapi_sm->ap_wapi_ie, wapi_sm->ap_wapi_ie_len);
            return -1;
        }
    }
    ...
    if(payload[0] & BIT(4))//it is rekey
        wapi_supplicant_key_negotiation_state_report(WPA_COMPLETED);
    else
        wapi_supplicant_key_negotiation_state_report(WPA_GROUP_HANDSHAKE);
    return 0;
}
...
static int waigroup_multicast_1_2(struct wapi_asue_st* wpa_s, u8* payload, int len)
{
    ...
    iwn_wpa_printf(MSG_DEBUG, "waigroup_multicast_1_2 msg len = %d", len);
    iwn_wpa_hexdump(MSG_DEBUG, "  msg = \n", payload, len);
    wpa_printf(MSG_DEBUG, "WPA: in %s:%d", __func__, __LINE__);
    ...
    if(payload[0] & BIT(5))
    {
        wpa_printf(MSG_DEBUG, "WPA: STAKEY_NEG  is not supptable by ASUE");
        return -1;
    }
    pos += 1 + BKID_LEN;
#ifdef REKEY
    ...
    {
        wpa_printf(MSG_DEBUG, "WPA: MSKID  is not invalid");
        return -1;
    }
#endif
    ...
    {
        wpa_printf(MSG_DEBUG, "WPA: ADDID  is wrong");
        return -1;
    }

    if(check_iv(&sm->msksa, key_an_id) < 0)
    {
        wpa_printf(MSG_DEBUG, "WPA: Key annoucement identfier  is wrong");
        iwn_wpa_hexdump(MSG_DEBUG, "WPA: msk_ann_id", sm->msksa.msk_ann_id, WAI_IV_LEN);
        iwn_wpa_hexdump(MSG_DEBUG, "rx WPA: Key_an_id", key_an_id, WAI_IV_LEN);
        return -1;
    }
    ...
    if(usk == NULL)
    {
        wpa_printf(MSG_DEBUG, "WPA: Key annoucement uskid  is wrong");
        return -1;
    }
    hmac_sha256(payload, len-WAI_MIC_LEN, usk->mak, PAIRKEY_LEN, mic, WAI_MIC_LEN);
    ...
    {
        wpa_printf(MSG_DEBUG, "WPA: Multicast  announcement packet mic  is wrong");
        return -1;
    }
...
static int wapi_install_usk(struct wapi_asue_st *wpa_s)
{
	...
	{
		iwn_wpa_printf(MSG_WARNING, "WPA: Failed to set PTK to the driver");
		return -1;
	}
...
static int wapi_install_msk(struct wapi_asue_st *wpa_s, unsigned char *key_sc)
{
	...
	iwn_wpa_hexdump(MSG_DEBUG, "*WPA: KEYSC", key_sc, 16);
	if (0 != WIFI_group_key_set(msksa->msk, 32, mskid, key_sc))
	{
		iwn_wpa_printf(MSG_WARNING, "WPA: Failed to set MSK to the driver.");
		return -1;
	}
