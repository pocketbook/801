...
#ifdef CONFIG_CCX
int
wpa_supplicant_get_bss_channel(struct wpa_supplicant *wpa_s, u8 *bssid)
{
	int channel = 2412;
#if 0
	...
	while ((ssid->proto & WPA_PROTO_RSN) && rsn_ie) {
		    ...
		    (ie.group_cipher & (WPA_CIPHER_WEP40 | WPA_CIPHER_WEP104)))
		{
			...
			return 1;
                ...
                if (!(ie.group_cipher & (WPA_CIPHER_WEP40 | WPA_CIPHER_WEP104)) &&
		    ((ssid->key_mgmt & WPA_KEY_MGMT_NONE) &&
		     (ssid->wep_key_len[ssid->wep_tx_keyidx] > 0))) {
			wpa_dbg(wpa_s, MSG_DEBUG, "   skip - cipher mismatch");
			return 0;
		}
...
#ifdef CONFIG_WAPI
    wapi_ie = wpa_scan_get_ie(bss, WLAN_EID_WAPI); //WLAN_EID_WAPI
    wapi_ie_len = wapi_ie ? wapi_ie[1] : 0;
    while ((ssid->proto & WPA_PROTO_WAPI) && wapi_ie_len > 0) {
...
/*important codes*/
		...
		wapi_ie = wpa_scan_get_ie(bss, WLAN_EID_WAPI); //WLAN_EID_WAPI
		wapi_ie_len = wapi_ie ? wapi_ie[1] : 0; 
		wpa_printf(MSG_DEBUG, "%d: " MACSTR " wapi_ie_len=%lu ",
					  ...
					  union wpa_event_data *data)
{
	...
	while (p && l >= 2) {
		...
		if (p[0] == WLAN_EID_CCKM && p[1] >= 4 &&
		     (os_memcmp(&p[2], "\x00\x40\x96\x00", 4) == 0)) {
			if (wpa_sm_set_cckm_resp_ie(wpa_s->wpa, p, len))
				break;
			cckm_found = 1;
			break;
		}
	...
	if (!cckm_found && data->assoc_info.resp_ies)
		wpa_sm_set_cckm_resp_ie(wpa_s->wpa, NULL, 0);
#endif
...
#if 0 /* defined(ANDROID) but not working for CCX */
	if (!wpa_found)
#else
				       ...
				       union wpa_event_data *data)
{
	...
	if ((wpa_s->wpa_state==WPA_4WAY_HANDSHAKE) &&
		...
		/* Fix incorrect password dialog doesn't show up issue*/
		return ; 
        }
#endif
#ifdef CONFIG_CCX
	if (!ft_completed && wpa_sm_is_cckm_resp_ie_set(wpa_s->wpa)) {
		int cckm_completed = 0;
		wpa_drv_get_bssid(wpa_s, bssid);
		if (cckm_process_response(wpa_s->wpa, bssid) == 0)
			cckm_completed = cckm_fr_is_completed(wpa_s->wpa);
		if (cckm_completed) {
			wpa_supplicant_cancel_scan(wpa_s);
			wpa_supplicant_cancel_auth_timeout(wpa_s);
			os_memcpy(wpa_s->bssid, bssid, ETH_ALEN);
			os_memset(wpa_s->pending_bssid, 0, ETH_ALEN);
			wpa_msg(wpa_s, MSG_DEBUG, "(Re)Associated to BSSID="
...
/*important codes*/
	...
	/*
		Addtional fix for EV 94621.

		When p2p_connection/disconnection is repeated, sometimes 'EAP_FAILURE' state is abruptly entered
		as soon as assoc_req/assoc_res is made for WPS session, leading to p2p_connection failure.
		This comes from that 'idleWhile' variable used by EAP machine & EAPOL machine of supplicant is only one time-initialized at 'eapol_sm_init'
		which are also only one-time called in supplicant life time in the current 0.8.x wpa_supplicant code. 

		To solve this issue, new functions to refresh 'sm->ClientTimeout' & 'eapol.idleWhile' to its original value in association is made.
		With this refreshing, the p2p_connection failure coming from the abrupt 'EAP_FAILURE' state entering does not happen.
	*/
    eapol_sm_notify_eap_timer_refresh(wpa_s->eapol);
#ifdef CONFIG_WAPI
	}
#endif
...
#endif /* CONFIG_WAPI, TODO: confirm*/
		...
		if (wpa_supplicant_driver_init(wpa_s) < 0) {
			wpa_msg(wpa_s, MSG_INFO, "Failed to initialize the "
               ...
               wpa_drv_p2p_capa_init(wpa_s);
                ...
                wpa_printf(MSG_INFO, "%s : wpas_p2p_init success", __func__);
#endif
			  ...
			  union wpa_event_data *data)
{
	...
	case EVENT_P2P_SD_RESPONSE:
		...
		break;
        case EVENT_P2P_INVITATION_SENT_RESULT:
                wpas_invitation_result(wpa_s, data->p2p_invite_sent_result.status,
                data->p2p_invite_sent_result.bssid);
                break;
        case EVENT_P2P_INVITATION_RCVD_RESULT:
                wpas_invitation_received(wpa_s, data->p2p_invite_rcvd_result.sa,
                     data->p2p_invite_rcvd_result.bssid,
                     data->p2p_invite_rcvd_result.ssid,
                     data->p2p_invite_rcvd_result.ssid_len,
                     data->p2p_invite_rcvd_result.go_dev_addr,
                     data->p2p_invite_rcvd_result.status,
                     data->p2p_invite_rcvd_result.op_freq);
                break;
#endif /* CONFIG_P2P */
...
#ifdef CONFIG_CCX
	case EVENT_CHANNEL:
		wpa_s->apinfo.channel = data->channel.channel;
		break;
#endif
	default:
