				...
				const char *value)
{
	...
	hex_len = os_strlen(value);
	psk_len = hex_len / 2;
    if ((hex_len > sizeof(ssid->psk)*2) || (hex_len % 2) || hexstr2bin(value, ssid->psk, psk_len) ||
	    value[psk_len * 2] != '\0') {
#else
...
#ifdef CONFIG_WAPI
	ssid->psk_set = psk_len;
    wpa_hexdump_key(MSG_MSGDUMP, "PSK", ssid->psk, psk_len);
#else
	ssid->psk_set = 1;
				     ...
				     const char *value)
{
	...
	if (val == -1)
		return -1;
	if (val & ~(WPA_CIPHER_CCMP | WPA_CIPHER_TKIP 
#ifdef CONFIG_WAPI
				| WPA_CIPHER_SMS4
#endif
				| WPA_CIPHER_NONE)) {
					   ...
					   const char *driver_param)
{
...
#ifdef CONFIG_CCX
	config->ccx_version = DEFAULT_CCX_VERSION;
#endif
	...
	config->p2p_max_clients = DEFAULT_MAX_NUM_STA;
