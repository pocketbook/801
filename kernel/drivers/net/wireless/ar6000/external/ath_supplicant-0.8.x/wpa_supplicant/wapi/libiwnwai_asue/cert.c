...
void *iwn_x509_get_pubkey(void *cert_st)
{
    ...
    do
    {
        ...
        {
			iwn_wpa_printf(MSG_ERROR,  "iwn_x509_get_pubkey: '%s', '%d' ", __FILE__, __LINE__);
			fail_flag = 1;
            break;
        }
...
void cert_obj_unregister(const struct cert_obj_st_t *cert_obj)
{
	...
	if ((cert_objs[cert_obj->cert_type] != NULL) && (cert_objs[cert_obj->cert_type] != cert_obj))
	{
		iwn_wpa_printf(MSG_ERROR,"cert_obj address 	%p\n", cert_obj);
		iwn_wpa_printf(MSG_ERROR,"obj address 		%p\n", cert_objs[cert_obj->cert_type]);
		iwn_wpa_printf(MSG_ERROR,"%s: certificate object %s registered with a different template\n",
			__func__, cert_obj->cert_name);
		return;
	}
