...
/**************************** sha.h ****************************/ /******************* See RFC 4634 for details ******************/
...
void KD_hmac_sha256(unsigned char* t, unsigned tl, unsigned char* k, unsigned kl, unsigned char* o, unsigned ol)
{
	...
	{
		ecc_hmac_sha256(t, tl, k, kl, &o[i*SHA256_DIGEST_SIZE], SHA256_DIGEST_SIZE);
		t = &o[i*SHA256_DIGEST_SIZE];
		tl = SHA256_DIGEST_SIZE;
	}
	if(length>0)	ecc_hmac_sha256(t, tl, k, kl, &o[i*SHA256_DIGEST_SIZE], ol);
}
...
static void test2()
{
	...
	ecc_hmac_sha256(in1, 32, k1, 32, out, 32);
	v1 = GetDatSum(out, 32);//ok = 2

	ecc_hmac_sha256(in2, 32, k2, 32, out, 32);
...
static void test3()
{
	...
	ecc_hmac_sha256(in1, sizeof(in1), k1, sizeof(k1), out, 32);
