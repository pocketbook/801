...
#ifdef CONFIG_WAPI
    if (!(ssid->key_mgmt & WPA_KEY_MGMT_WAPI_PSK) && ssid->psk_set && !ssid->passphrase) {
		if (ssid->psk_set == 32) {
			ssid->psk_set = 1; /* reset to 1 to set len as PMK_LEN for wpa networks */
		} else {
			wpa_printf(MSG_ERROR, "WPA-PSK hex lenghti is not equal to %d", 32);
			errors++;
		}		
	}
...
static void wpa_config_write_global(FILE *f, struct wpa_config *config)
{
	...
	if (config->ap_scan != DEFAULT_AP_SCAN)
		fprintf(f, "ap_scan=%d\n", config->ap_scan);
#ifdef CONFIG_CCX
	if (config->ccx_version != DEFAULT_CCX_VERSION)
		fprintf(f, "ccx_version=%d\n", config->ccx_version);
#endif
	...
	if (config->p2p_max_clients != DEFAULT_MAX_NUM_STA)
		fprintf(f, "p2p_max_clients=%u\n", config->p2p_max_clients);
