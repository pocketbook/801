...
#ifdef CONFIG_CCX
"\nCopyright (c) 2009-2011, embWiSe Technologies"
#endif
;
...
#else
"This product is released under BSD License and includes proprietary\n"
"additions for CCX\n"
#endif
				       ...
				       struct wpa_ssid *ssid)
{
	...
	wpa_sm_set_ccx_version_ie(wpa_s->wpa, NULL, 0);
#endif
			      ...
			      enum wpa_states state)
{
...
#ifdef ANDROID
	int network_id = -1;
	enum wpa_states reported_state = state;
...
#ifdef ANDROID
    /* Here is a WAR to match supplicant 0.8.x new state into 
     * Android framework state. It works for Android version <= 3.0
     * Android may adapt 0.8.x supplicant in the future and we should remove 
     * the following patch 
     */ 
	if (reported_state!=WPA_AUTHENTICATING && 
			reported_state!=WPA_INTERFACE_DISABLED) {
		if (reported_state>WPA_AUTHENTICATING)
			reported_state-=2;
		else if (reported_state>WPA_INTERFACE_DISABLED)
			reported_state-=1;
		wpa_msg(wpa_s, MSG_INFO, 
			WPA_EVENT_STATE_CHANGE "id=%d state=%d BSSID=" MACSTR, 
	...
	if (wpa_s->wpa_state==WPA_COMPLETED && 
	    (wpa_s->key_mgmt!=WPA_KEY_MGMT_NONE && state==WPA_GROUP_HANDSHAKE)) {
		/* Fix Android GUI obtaining IP issue when rekeying */
	...
	if ((wpa_s->wpa_state == WPA_COMPLETED) &&
		...
		struct os_time t;
		os_get_time(&t);
		/* do not reset the time, until new connection */
		wpa_s->apinfo.disconnect_time = t.sec;
	}
#endif
	...
	if (state == WPA_COMPLETED && wpa_s->new_connection) {
		...
		if (wpa_key_mgmt_cckm(wpa_s->key_mgmt)) {
			if (wpa_s->apinfo.is_valid &&
					os_memcmp(wpa_s->apinfo.bssid,
						wpa_s->bssid, ETH_ALEN)) {
				ccx_send_report(wpa_s);
			}
			os_memcpy(wpa_s->apinfo.bssid, wpa_s->bssid, ETH_ALEN);
			wpa_s->apinfo.ssid_len = wpa_s->current_ssid->ssid_len;
			os_memcpy(wpa_s->apinfo.ssid, wpa_s->current_ssid->ssid,
					wpa_s->current_ssid->ssid_len);
			wpa_s->apinfo.disconnect_time = 0;
			wpa_s->apinfo.is_valid = 1;
#ifndef CONFIG_NO_SCAN_PROCESSING
			if (wpa_s->apinfo.channel == 0) {
				wpa_s->apinfo.channel =
					wpa_supplicant_get_bss_channel(wpa_s,
							wpa_s->bssid);
			}
#endif
		} else {
			wpa_s->apinfo.is_valid = 0;
			wpa_s->apinfo.channel = 0;
		}
#endif
	...
	wpa_s->wpa_state = state;
        if((old_state == WPA_COMPLETED) && (state == WPA_DISCONNECTED) &&
          (wpa_s->drv_flags & WPA_DRIVER_FLAGS_P2P_CAPABLE)) {
            ...
            if(ssid) {
                wpa_drv_deauthenticate(wpa_s, wpa_s->bssid, WLAN_REASON_DEAUTH_LEAVING);
			      ...
			      u8 *wpa_ie, size_t *wpa_ie_len)
{
	...
	if (bss_rsn && (ssid->proto & WPA_PROTO_RSN) &&
		...
		proto = WPA_PROTO_WPA;
#ifdef CONFIG_WAPI
	} else if ((ssid->proto & WPA_PROTO_WAPI) && bss && wpa_bss_get_ie(bss, WLAN_EID_WAPI)) {
	...
	if (0) {
		...
		wpa_dbg(wpa_s, MSG_DEBUG, "WPA: using KEY_MGMT CCKM");
#endif
			      ...
			      struct wpa_bss *bss, struct wpa_ssid *ssid)
{
...
#ifdef CONFIG_CCX
	int reassociate = wpa_s->reassociate;
#endif
#ifdef CONFIG_IBSS_RSN
	ibss_rsn_deinit(wpa_s->ibss_rsn);
	wpa_s->ibss_rsn = NULL;
...
#ifdef ANDROID
		wpa_s->link_speed = wpa_bss_get_max_rate(bss) * 500000;
...
#ifdef CONFIG_CCX
	wpa_sm_set_cckm_params(wpa_s->wpa, wpa_s->reassociate);
	if (reassociate && bss) {
		wpa_msg(wpa_s, MSG_INFO, "Re-associate with " MACSTR,
				MAC2STR(bss->bssid));
		wpa_sm_set_cckm_req_ie(wpa_s->wpa, NULL, 0);
	}
    ...
    if (ssid->proto & WPA_PROTO_WAPI) {
        ...
        wapi_supplicant_associate(wpa_s, bss, ssid, &params);
		old_ssid = wpa_s->current_ssid;
