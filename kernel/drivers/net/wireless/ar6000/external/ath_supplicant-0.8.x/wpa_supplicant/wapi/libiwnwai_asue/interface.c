...
static void
WAI_RX_wai(const u8* src_addr, const u8* buf, size_t len)
{
    struct wapi_asue_st* wpa_s = s_asue;
    int t = wpa_s->ap_type;
    ...
    if (temp_rxbuf) {
        ...
        iwn_wapi_sm_rx_wai(wpa_s, src_addr, temp_rxbuf->data,
                           temp_rxbuf->data_len);
    }
}
...
int WAI_CNTAPPARA_SET(const CNTAP_PARA* pPar)
{
	...
	if (AUTH_TYPE_NONE_WAPI != pPar->authType)
	{
		...
		unsigned char buf_ie[64] =
		    {0x44, 22, 1, 0, 1 ,0, 0, 0x14, 0x72, 1, 1,0,0,0x14,0x72,1, 0,0x14,0x72,1, 0, 0, 0, 0};/* little endian*/
		...
		WIFI_WAI_IE_set(buf_ie, buf_ie[1] + 2);

		memcpy(wpa_s->wapi_sm->assoc_wapi_ie, buf_ie, buf_ie[1] + 2);
		wpa_s->wapi_sm->assoc_wapi_ie_len = (u8)(buf_ie[1] + 2);
	}
	...
	else
	{
		...
		if (AUTH_TYPE_WAPI_PSK == pPar->authType)
		{
			...
			else		/* HEX mode for PSK */
			{
				...
				ret = iwn_password_bk_derivation(buff, pPar->para.kl, wpa_s->psk_bk);
			}
			iwn_wpa_hexdump(MSG_DEBUG, "WAI: ", wpa_s->psk_bk, 16);
		}
		else	/* open mode*/
		{
			ret = 0;
...
void WAI_Msg_Input(CONN_STATUS action,
                   const MAC_ADDRESS* pBSSID,
                   const MAC_ADDRESS* pLocalMAC,
                   unsigned char *assoc_ie,
                   unsigned char assoc_ie_len)
{
	...
	else if (CONN_DISASSOC == action)/* DISCONN ASSOC */
    {
        wpa_s->wapi_state = WAPISM_N0_ASSOC;
        wpa_s->usk_updated = 0;
...
{
	struct wapi_asue_st* wpa_s = s_asue;
	WAI_RX_wai((const u8*)wpa_s->bssid, (const u8*)pbuf, (size_t)length);
	return 0;
}
