				    ...
				    const u8 *ie, size_t ie_len)
{
	...
	if (data.key_mgmt & WPA_KEY_MGMT_CCKM) {
		ret = os_snprintf(pos, end - pos, "%sCCKM", first ? "" : "+");
		if (ret < 0 || ret >= end - pos)
			return pos;
		pos += ret;
		first = 0;
	...
	struct wpa_supplicant *wpa_s, char *buf, size_t buflen)
{
	...
	dl_list_for_each(bss, &wpa_s->bss_id, struct wpa_bss, list_id) {
		/*
		 * Do ssid filtering, if applicable
		 */
		if (wpa_s->conf->filter_ssids) {
			ssid = wpa_s->conf->ssid;
			/*
			 * Go through the list of configured SSIDs.  See if
			 * the SSID of the current list element (bss->ssid)
			 * matches any of the configured SSIDs (ssid->ssid).
			 * If we have a match, break out of the loop and add
			 * a string representing that SSID to the scan results.
			 * If no match, keep searching the list of configured
			 * SSIDs until we either find a match or all configured
			 * SSIDs are exhausted.
			 */
			while(ssid) {
			...
			/*
			 * No match.  Go on to the next SSID without adding this
			 * one to the scan results.
			 */
			continue;
		}
 ssid_match:
...
static int p2p_ctrl_invite_auth(struct wpa_supplicant *wpa_s, char *cmd)
{
	char *pos;
	int auth;
        u8 peer[ETH_ALEN];
	...
	if (pos) {
		...
		if (wpa_s->drv_flags & WPA_DRIVER_FLAGS_P2P_MGMT) {
			return wpa_drv_p2p_auth_invite(wpa_s, auth, peer);
		} else {
			/* Only supported if P2P managed in driver, for now */
			return -1;
		}
	}
	return -1;
...
static int p2p_ctrl_invite(struct wpa_supplicant *wpa_s, char *cmd)
{
	...
	if (os_strncmp(cmd, "auth=", 5) == 0)
		return p2p_ctrl_invite_auth(wpa_s, cmd + 5);
...
#ifdef CONFIG_CCX
static int wpa_supplicant_ctrl_iface_ccx_version(
		struct wpa_supplicant *wpa_s, char *cmd)
{
	int ccx_version = atoi(cmd);

	if (ccx_version < 1 || ccx_version > 4)
		return -1;
	wpa_s->conf->ccx_version = ccx_version;
	return 0;
}
#endif
					 ...
					 char *buf, size_t *resp_len)
{
	...
	} else if (os_strncmp(buf, WPA_CTRL_RSP, os_strlen(WPA_CTRL_RSP)) == 0)
	{
		...
		if (os_strlen(buf) > 10 && reply_len == 0) {
			int has_bl = (wpa_s->blacklist!=NULL);
			struct wpa_blacklist *bl = wpa_s->blacklist;
			if ((has_bl && os_strncmp(buf+10, "clear", 5) == 0) ||
...
#else /* ANDROID will shutdown interface if we are disconnected. skip this */
		if (wpa_s->wpa_state == WPA_INTERFACE_DISABLED)
			reply_len = -1;
		else 
#endif /* ANDROID */
		...
		if (wpa_supplicant_ctrl_iface_ap_scan(wpa_s, buf + 8))
			reply_len = -1;
#ifdef CONFIG_CCX
	} else if (os_strncmp(buf, "CCX_VERSION ", 8) == 0) {
		if (wpa_supplicant_ctrl_iface_ccx_version(wpa_s, buf + 8))
			reply_len = -1;
#endif
		...
		if (ap_ctrl_iface_deauthenticate(wpa_s, buf + 15))
			reply_len = -1;
	} else if (os_strncmp(buf, "DISASSOCIATE ", 13) == 0) {
		if (ap_ctrl_iface_disassociate(wpa_s, buf + 13))
			reply_len = -1;
#endif /* CONFIG_AP */
