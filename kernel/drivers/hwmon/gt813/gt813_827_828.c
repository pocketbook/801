...
/*
[   90.331514] <<-GTP-ERROR->>[349]I2C transfer error. errno:-110
[   90.331522]  
[   90.331533] <<-GTP-FUNC->>[236]Func:gtp_irq_enable
[   90.331562] <<-GTP-FUNC->>[504]Func:goodix_ts_irq_handler
[   90.331571] <<-GTP-FUNC->>[211]Func:gtp_irq_disable
[   90.331589] <<-GTP-FUNC->>[338]Func:goodix_ts_work_func
[   90.331597] <<-GTP-FUNC->>[89]Func:gtp_i2c_read
*/
...
void dump_i2c_msg(struct i2c_msg *m)
{
	int i;
	
	printk("addr:%x len:%u flags:%x data:", m->addr, m->len, m->flags);
...
static void gtp_touch_down(struct goodix_ts_data* ts,s32 id,s32 x,s32 y,s32 w)
{
    ...
    GTP_SWAP(x, y);
      ...
      GTP_DEBUG("%s,X1:%d, Y1:%d ",__func__, x, y);
      y= GTP_USER_Y_MAX -y;
#endif
    ...
    GTP_DEBUG("%s,ID:%d, X:%d, Y:%d, W:%d",__func__, id, x, y, w);
}
...
static void goodix_ts_work_func(struct work_struct *work)
{
    ...
    GTP_DEBUG("pre_touch:%02x, finger:%02x.", ts->pre_touch, finger);

#if GTP_ICS_SLOT_REPORT
    if (ts->pre_touch || touch_num)
    {
        s32 pos = 0;
    ...
    //printk("<0> setting last activity goodix\n");


    ts->pre_touch = finger & 0x1f;
...
static enum hrtimer_restart goodix_ts_timer_handler(struct hrtimer *timer)
{
	...
	schedule_delayed_work(&ts->work, HZ/20);
...
static s8 gtp_request_irq(struct goodix_ts_data *ts)
{
	...
	/*add for it can wakup the screen */
	//enable_irq_wake(ts->client->irq);
	   printk(" request irq num : %d\n",ts->client->irq);
        gtp_irq_disable(ts);
        ts->use_irq = 1;
        return 0;
    }
...
static int goodix_ts_suspend(struct device *dev)
{
	...
	struct goodix_ts_data *priv = dev_get_drvdata(&client->dev);
	s8 ret = -1;

	GTP_DEBUG_FUNC();

	//    printk("<0> suspending goodix\n");
	//printk("%s %s %d \n",__FILE__,__func__,__LINE__); 

//	if ((jiffies - lastactivity_jf) < 1 * HZ) {
//	      printk(KERN_WARNING "cancelling suspending goodix TS\n");
//	     return -EBUSY;
//	}

	//The device does not go to sleep mode, while any touch point are active
	if (priv->pre_touch)
		return -EBUSY;
...
#if GTP_ESD_PROTECT
		priv->gtp_is_suspend = 1;
		cancel_delayed_work_sync(&gtp_esd_check_work);
#endif
		if (priv->use_irq)
		{
			gtp_irq_disable(priv);
		}
		else
		{
			hrtimer_cancel(&priv->timer);
		}
		ret = gtp_enter_sleep(priv);
		if (ret < 0)
		{
			GTP_ERROR("GTP early suspend failed.");
		}
	}else if(iXrzSuspendMode ==1)
	{
		...
		//printk("GT813,enter suspend mode: priv->use_irq =%d,priv->client->irq=%d\n",
		//priv->use_irq, priv->client->irq);
		if (priv->use_irq)
...
#if GTP_ESD_PROTECT
		priv->gtp_is_suspend = 0;
		queue_delayed_work(gtp_esd_check_workqueue, &gtp_esd_check_work, GTP_ESD_CHECK_CIRCLE);
#endif
	}else if(iXrzSuspendMode == 1){
		if (priv->use_irq)
			disable_irq_wake(priv->client->irq);
	}
	return 0;
}

static const struct dev_pm_ops goodix_ts_pm_ops = {
