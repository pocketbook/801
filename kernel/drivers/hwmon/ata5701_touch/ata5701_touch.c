...
static void ata5701_work_func(struct work_struct *work)
{
   ...
   ataData = container_of(work, struct ata5701_data, work);

	uData = i2c_smbus_read_byte_data(ataData->client, ATA5701_COMMAND_GETGRADE);
	touch_bitmap = i2c_smbus_read_byte_data(ataData->client, ATA5701_COMMAND_GETBITMAP);

	ATA5701_DEBUG("%s,uData=%d, touch_bitmap = %d, ",__func__, uData,touch_bitmap);

	if (uData >= 0 && uData <40)	{

		input_report_abs(ataData->input_dev, ABS_X, uData);

		if (is_pushed && touch_bitmap == 0) {
			input_report_key(ataData->input_dev, BTN_TOUCH,0);
			is_pushed = false;
		} else if (!is_pushed && touch_bitmap > 0) {
			input_report_key(ataData->input_dev, BTN_TOUCH,1);
			is_pushed = true;
		}
...
static s8 ata5701_request_irq(struct ata5701_data *ataData)
{
   ...
   ATA5701_DEBUG_FUNC();	
    ...
    ret  = request_irq(ataData->client->irq, 
        ...
        ATA5701_DEBUG("%s,ret=:%d, ",__func__, ret);
	if(ret ==0)
	{
...
static s8 ata5701_request_input_dev(struct ata5701_data *ataData)
{
   ...
   ATA5701_DEBUG_FUNC();	
    ...
    input_set_abs_params(ataData->input_dev, ABS_X, 0, 39, 0, 0);
...
static int ata5701_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    client->addr = 0x33;
    ATA5701_INFO("%s,I2C addr:%x",__func__, client->addr);
