...
static int control_home=0;
static int control_sendout=0;
static int scancode_tab=0;
/*!
 * Keypad Module Name
 */
	...
	if (press) {
		if ((3 == col) && ((3 == row) ||
			...
			if (first_set == 0) {
				...
				pr_debug("Press0 (%d, %d) scan=%d Kpress=%d\n",
			...
			if ((0 == (cur_rcmap[3] & BITSET(0, 3))) &&
				...
				kpp_dev.iKeyState = KStateUp;
				flag = 1;
				pr_debug("Press1 (%d, %d) scan=%d Kpress=%d\n",
					 row, col, scancode, KPress);
			}
		}
	} else {
		if ((flag == 0) && (3 == col)
			...
			if (first_set == 0) {
				...
				pr_debug("Release0 (%d, %d) scan=%d Kpress=%d\n",
					 row, col, scancode, KPress);
			}
		} else {
			/*
			...
			if ((0 == (prev_rcmap[3] & BITSET(0, 3))) &&
				...
				flag = 0;
				kpp_dev.iKeyState = KStateDown;
				pr_debug("Release1 (%d, %d) scan=%d Kpress=%d\n",
					 row, col, scancode, KPress);
			}
		}
	}
...
static int mxc_kpp_scan_matrix(void)
{
		...
		for (col = 0; col < kpp_dev.kpp_cols; col++) {
			if ((diff >> col) & 0x1) {
				...
				if ((prev_rcmap[row] & BITSET(0, col)) == 0) {
					...
					pr_debug("Release2 (%d, %d) scan=%d Kpress=%d\n",
					     row, col, scancode, KPress);
	...
	case KStateDown:
		if (KPress) {
			if(control_home<35){
			   control_home ++;
			}else if((11==scancode_tab)&&(0==control_sendout)){
			        scancode=8;//home button			    
				input_event(mxckbd_dev, EV_KEY,
				    mxckpd_keycodes
				    [scancode], 1);
				...
				input_event(mxckbd_dev, EV_KEY,
					    mxckpd_keycodes
					    [scancode], 0);
				//printk("%s %s %d key home press =%d !\n",__FILE__,__func__,__LINE__,mxckpd_keycodes[scancode]);
				control_sendout=1;
			}
			...
			for (row = 0; row < kpp_dev.kpp_rows; row++) {
				for (col = 0; col < kpp_dev.kpp_cols; col++) {
					if ((release_scancode[row][col] != -1)) {
						...
						if((1!=control_sendout)&&(11==scancode))
						{
							input_event(mxckbd_dev, EV_KEY,
							    mxckpd_keycodes
							    [scancode], 1);
...
{
	printk(KERN_INFO "MXC keypad loaded1\n");
