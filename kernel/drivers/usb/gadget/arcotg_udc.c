...
#include "../../staging/dream/include/linux/wakelock.h"
...
//#define USB_CHARGER_DETECT
//#define  USB_DCDC_CHARGER_DETECT
...
extern int mx50_rdp_set_charger_gpio(int gValue);
extern int mx50_rdp_get_charger_gpio(void);
#if defined(CONFIG_MXC_MC13892_BATTERY)
extern int set_DCDC_charger_state(bool bState);
//extern int pmic_reset_usb_charger_current(void);
#endif
...
static void dr_clk_gate(bool on);
/* The functions & global variables for usb charger detection */

#if defined(CONFIG_CHARGING_CURRENT_CONTROL) 
#define CONFIG_FSL_USB_CHARGER   1
#endif

#if CONFIG_FSL_USB_CHARGER
#define USB_CHARGER_DETECT_TIME   15000
//#undef pr_debug
//#define pr_debug printk
static void check_usb_charger(void);
static void enable_usb_charger(void);
static void disable_usb_charger(void);
static void check_charger_fn(unsigned long _udc);
static void check_charger_debounce_fn(unsigned long _udc);
static void check_long_duration_plug_in_fn(unsigned long _udc);
static void usb_charger_handler(void);
enum usb_battery_charger {
	CHARGER_NONE,
        CHARGER_DCP, /* Max 1.5A charging allowed */
        CHARGER_CDP, /* Max 900mA charging allowed */
        CHARGER_ACTIVE_HOST, /* Max 100mA charging allowed before finishing
			      * enumeration, can be up to 500mA
			      * after enumeration */
	CHARGER_SUSPEND_HOST /* not allowed to charge */
} fsl_usb_battery_charger = CHARGER_NONE;

//struct task_struct *charger_thread;
static struct timer_list check_charger_timer;
static struct timer_list check_charger_debounce_timer;
static struct timer_list check_long_duration_plug_in_timer;
static int check_charger_timer_exec_counter = 1;
static int detect_charger_counter;
static struct wake_lock detect_usb_charger_wake_lock;
#endif
//static bool bConnectPC = false;
...
#if defined(CONFIG_CHARGING_GPIO_CONTROL)
static int  charger_current_state(bool bCur)
{
   if(bCur)
   	 mx50_rdp_set_charger_gpio(1);
   else
   	mx50_rdp_set_charger_gpio(0);

   return 0;
}
#elif defined(CONFIG_MXC_MC13892_BATTERY)
static int  charger_current_state(bool bCur)
{
    //printk(KERN_INFO"%s,%d bCur=%d fsl_usb_battery_charger=%d",__func__,__LINE__,bCur,fsl_usb_battery_charger);
   set_DCDC_charger_state(bCur); 
   return 0;
}
#else
static int  charger_current_state(bool bCur){}
...
#if CONFIG_FSL_USB_CHARGER
static struct work_struct chgdet_work;
static struct work_struct chgdet_stop_work;
static int chgdet_clk_on;
/*
 *	Usb Charger Used Function (Need to move to MSL in the future)
 * 	Please check the usb charger flow chart for detail.
 */
static void enable_usb_charger(void)
{
	u32 otg_phy_ctrl;
	/* 
	 * Chgrdeton=1; udelay(300); Chgrdeten=1
	 */
	otg_phy_ctrl = fsl_readl(&dr_regs->phyctrl0);
	fsl_writel(otg_phy_ctrl | (1 << 23), &dr_regs->phyctrl0);
	udelay(300);
	otg_phy_ctrl = fsl_readl(&dr_regs->phyctrl0);
	fsl_writel(otg_phy_ctrl | (1 << 24), &dr_regs->phyctrl0);
}
...
static void disable_usb_charger(void)
{
	u32 otg_phy_ctrl;
	/* 
	 * Chgrdeton=0; Chgrdeten=0
	 */
	otg_phy_ctrl = fsl_readl(&dr_regs->phyctrl0);
	fsl_writel(otg_phy_ctrl & (~(1 << 23)) & (~(1 << 24)), &dr_regs->phyctrl0);
}
extern bool bConnectState ;
static bool is_usb_charger(void)
{
	 ...
	 * wen modify 2013-7-4 
	 */
	//return ((readl(&dr_regs->phyctrl0) & (1 << 2)) != 0) ? true : false;
	return bConnectState;
}

static void check_usb_charger(void)
{
	if(wake_lock_active(&detect_usb_charger_wake_lock)==0)
	{
	    wake_lock(&detect_usb_charger_wake_lock);
	}
	detect_charger_counter++;
	enable_usb_charger();
	/* Check charger connection 50ms later */
	mod_timer(&check_charger_timer,jiffies + msecs_to_jiffies(50));
	/* Check long duration plug in later */
	mod_timer(&check_long_duration_plug_in_timer,jiffies + msecs_to_jiffies(USB_CHARGER_DETECT_TIME));
}
...
static void check_charger_fn(unsigned long _udc)
{
	...
	if(!chgdet_clk_on)
	{
		check_charger_timer_exec_counter = 1;
		return;
	}
	...
	{
		check_charger_timer_exec_counter = 1;
		schedule_work(&chgdet_stop_work);
		return;		
	}
	
	if (is_usb_charger()){
		mod_timer(&check_charger_debounce_timer, jiffies + msecs_to_jiffies(15));
	}else {
		if (check_charger_timer_exec_counter > 18) {
			/* connect to Host */
			disable_usb_charger();
			/* Pull up DP */
			...
			mod_timer(&check_charger_timer, jiffies + msecs_to_jiffies(50));
			check_charger_timer_exec_counter++;
		}
	}
}
...
static void check_long_duration_plug_in_fn(unsigned long _udc)
{
	...
	if(!chgdet_clk_on)
	{
		if((fsl_usb_battery_charger == CHARGER_DCP )
			|| (fsl_usb_battery_charger ==  CHARGER_CDP)){
			charger_current_state(true);
		 }else if(fsl_usb_battery_charger== CHARGER_ACTIVE_HOST){
		      charger_current_state(false);
		 }
               check_charger_timer_exec_counter = 1;
	       return;
	 }
	...
	temp = fsl_readl(&dr_regs->otgsc);
      ...
      if (((fsl_usb_battery_charger == CHARGER_SUSPEND_HOST) \
			|| (fsl_usb_battery_charger == CHARGER_NONE))\
		       	&& (temp & OTGSC_B_SESSION_VALID) \
			&& (detect_charger_counter == 1)) {
		/* Pull down DP */
		temp= fsl_readl(&dr_regs->usbcmd);
		temp &= ~USB_CMD_RUN_STOP;
		fsl_writel(temp, &dr_regs->usbcmd);

		/* detect charger again */
		//check_usb_charger();
		schedule_work(&chgdet_work);
	}

}

/* To differentiate 
 * CDP(Charging Downstream Port) or DCP(USB Dedicated Charger)
 */
static void usb_charger_handler(void)
{
	u32 temp;
	...
	if ((temp & PORTSCX_LINE_STATUS_BITS) == PORTSCX_LINE_STATUS_UNDEF) {
		//printk("%s: it is dedicated usb charger\n", __func__);
		fsl_usb_battery_charger = CHARGER_DCP;
		//printk(KERN_INFO "%s %d: fsl_usb_battery_charger = CHARGER_DCP\n", __func__,__LINE__);
	} else {
		//printk("%s: it is host charger\n", __func__);
		fsl_usb_battery_charger = CHARGER_CDP;
		//printk(KERN_INFO"%s %d: fsl_usb_battery_charger = CHARGER_CDP\n", __func__,__LINE__);
	}
	if(wake_lock_active(&detect_usb_charger_wake_lock)) {
		wake_unlock(&detect_usb_charger_wake_lock);
	}
	schedule_work(&chgdet_stop_work);
}
static void fsl_usb_charger_work_func (struct work_struct *work)
{
...
static void fsl_usb_charger_stop_work_func (struct work_struct *work)
{
    struct fsl_usb2_platform_data *pdata = udc_controller->pdata;
		...
		struct usb_ctrlrequest *setup)
{
	...
	case USB_REQ_SET_CONFIGURATION:
		...
		fsl_usb_battery_charger = CHARGER_ACTIVE_HOST;
		//printk(KERN_INFO"%s %d:fsl_usb_battery_charger = CHARGER_ACTIVE_HOST\n", __func__,__LINE__);
#endif
	     break;
...
static void suspend_irq(struct fsl_udc *udc)
{
	...
	/* connect to suspend host */
//#if CONFIG_FSL_USB_CHARGER
//	if(fsl_usb_battery_charger != CHARGER_NONE)
//	{
//		fsl_usb_battery_charger = CHARGER_SUSPEND_HOST;
//		printk(KERN_DEBUG "%s:fsl_usb_battery_charger = CHARGER_SUSPEND_HOST\n", __func__);
//	}
//	else
//	{
//		fsl_usb_battery_charger = CHARGER_CDP;
//		printk(KERN_DEBUG "%s:fsl_usb_battery_charger = CHARGER_CDP\n", __func__);
//	}
//#endif
}
...
#if CONFIG_FSL_USB_CHARGER
			fsl_usb_battery_charger = CHARGER_NONE;
			//printk(KERN_INFO "%s: fsl_usb_battery_charger = CHARGER_NONE\n", __func__);
			detect_charger_counter = 0;
#endif
			return false;
		}
	}
...
#if CONFIG_FSL_USB_CHARGER
/* Three timers to be used for usb charger detection */
	setup_timer(&check_charger_timer, check_charger_fn, \
		       	(unsigned long)udc_controller);
	setup_timer(&check_charger_debounce_timer,  \
			check_charger_debounce_fn, (unsigned long )udc_controller);
	setup_timer(&check_long_duration_plug_in_timer,  \
			check_long_duration_plug_in_fn, (unsigned long )udc_controller);

	wake_lock_init(&detect_usb_charger_wake_lock, WAKE_LOCK_SUSPEND, "fsl_usb_charger");
#endif
...
//#ifdef CONFIG_TEST_FSL_USB_CHARGER
//	//struct task_struct *charger_thread = kthread_run(usb_charger_thread, NULL, "fsl_usb_charger");
//        charger_thread = kthread_run(usb_charger_thread, NULL, "fsl_usb_charger");
//	int status;
//	status = IS_ERR(charger_thread) ? -1 : 0;
//	if (status)
//		printk(KERN_ERR "can't create kernel thread for usb charger\n");	
//#endif
...
#if CONFIG_FSL_USB_CHARGER
	/* usb charger related destroy work */
	del_timer_sync(&check_charger_timer);
	del_timer_sync(&check_charger_debounce_timer);
	del_timer_sync(&check_long_duration_plug_in_timer);
	wake_lock_destroy(&detect_usb_charger_wake_lock);
#endif
...
static bool is_usb_charger(void)
{
      ...
      */
      ...
      //ivalue= readl(&dr_regs->phyctrl0);
      //printk("%s %s %d ivalue =%d \n",__FILE__,__func__,__LINE__,ivalue);
      return ((readl(&dr_regs->phyctrl0) & (1 << 2)) != 0) ? true : false;          

}
static ssize_t fsl_otg_udc_charger_show(struct device *dev,
                struct device_attribute *attr, char *buf)
{
    bool result;
    enable_usb_charger();
    mdelay(50);/*wait 50ms*/
    result = is_usb_charger();
    disable_usb_charger();
	
	
    if(result){
	mx50_rdp_set_charger_gpio(1);
        return sprintf(buf, "charger is detected\n");
    }else{
	mx50_rdp_set_charger_gpio(0);
        return sprintf(buf, "USB host is detected\n");
    }
	
}

static DEVICE_ATTR(charger, 0444, fsl_otg_udc_charger_show, NULL);

bool my_DCDC_insert_state(void)
{
    bool breseult = false;
...
#if CONFIG_FSL_USB_CHARGER
	chgdet_clk_on = 0;
	INIT_WORK(&chgdet_work, fsl_usb_charger_work_func);
	INIT_WORK(&chgdet_stop_work, fsl_usb_charger_stop_work_func);
#endif
#ifdef POSTPONE_FREE_LAST_DTD
	last_free_td = NULL;
#endif
...
#if !(defined CONFIG_ARCH_MX35 || defined CONFIG_ARCH_MX25)
{
	...
	create_proc_file();
#if defined(USB_CHARGER_DETECT)
      sysfs_create_file(&(pdev->dev.kobj), &dev_attr_charger.attr);
#endif
	return 0;
...
#if defined(USB_CHARGER_DETECT)
	sysfs_remove_file(&(pdev->dev.kobj), &dev_attr_charger.attr);
#endif
	/* DR has been stopped in usb_gadget_unregister_driver() */
	remove_proc_file();

//	flush_workqueue(usb_gadget_queue);
//	destroy_workqueue(usb_gadget_queue);
...
static int fsl_udc_suspend(struct platform_device *pdev, pm_message_t state)
{
	...
	if (udc_controller->transceiver->gadget == NULL)
		return 0;
#endif
      ///enter suspend mode,we need set the mode as
      /// CHARGER_SUSPEND_HOST ?
