...
#define MC34708_BAT_VOLTAGE_UNIT_UV 4692
#define MC34708_BAT_CURRENT_UNIT_UA 7813
#define MC34708_CHG_VOLTAGE_UINT_UV 23474
#define MC34708_CHG_MIN_CURRENT_UA 3500
...
#define MC34708_DEV_NAME  "mc34708_battery"
...
static int ripley_battery_read_status(struct ripley_dev_info *di)
{
	...
	if (retval == 0){
		di->voltage_uV = di->voltage_raw * MC34708_BAT_VOLTAGE_UNIT_UV;
		printk("%s %s %d di->voltage_uV=%d\n",__FILE__,__func__,__LINE__,di->voltage_uV); 
	}
	...
	if (retval == 0) {
		if (di->current_raw & 0x200)
			di->current_uA =
			    ...
			    MC34708_BAT_CURRENT_UNIT_UA * (-1);
		else
			di->current_uA =
			    (di->current_raw & 0x1FF) * MC34708_BAT_CURRENT_UNIT_UA;
	}
...
static int __init ripley_battery_init(void)
{
	...
	ret = platform_driver_register(&ripley_battery_driver_ldm);
	printk("%s %s %d ret =%d\n",__FILE__,__func__,__LINE__,ret); 
	if (ret<0)
		printk(KERN_ERR "%s: failed to add mc34708 battary driver\n", __func__);
