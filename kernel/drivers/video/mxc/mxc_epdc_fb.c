...
#define DEFAULT_TEMP_AUTO_UPDATE_PERIOD  60 /*60 second*/
...
static bool full_updata_mode = false;
static bool full_updata_ok = false;
static bool screen_updata_flag = true;

extern int iXrzSuspendMode ;
//static  int my_mode_gc16=0;
...
static int mxc_epdc_earlysuspend_mode=0;
...
//static int mxc_epdc_fb_get_temp_index(struct mxc_epdc_fb_data *, int );
static bool FB_Resume_full_updata=true;
...
int COMBINE_WAVEFORM_MODE(int a, int b) {
	if (a == 2 || b == 2) return 2; // gc16
	if (a == 3 || b == 3) return 3; // gc4
	if (a == 1 || b == 1) return 1; // bw
	return b;
}
							   ...
							   bool use_test_mode, u32 np_val)
{
	...
	if((full_updata_mode == true) && (full_updata_ok == false) && (update_mode == 1))
	{
		full_updata_mode = false;
		update_mode = 1;
	}
	else if((full_updata_mode == true) && (full_updata_ok == true))
	{
		//printk("Jim_dream fb full_updata 2\n"); 
		full_updata_mode = false;
		update_mode = 1;
	}
	else
...
static void epdc_powerup(struct mxc_epdc_fb_data *fb_data)
{
	...
	*/
	...
	//regulator_enable(fb_data->v3p3_regulator);
	...
	{
		ret = regulator_enable(fb_data->tmst_regulator);
		if (IS_ERR((void *)ret)) {
			dev_err(fb_data->dev, "Unable to enable TMST regulator."
				"err = 0x%x\n", ret);
			mutex_unlock(&fb_data->power_mutex);
			return;
		}
...
static void epdc_powerdown(struct mxc_epdc_fb_data *fb_data)
{
	...
	*/
	...
	if(!IS_ERR(fb_data->tmst_regulator))
		regulator_disable(fb_data->tmst_regulator);
	...
	//regulator_disable(fb_data->v3p3_regulator); no need because it will be disabled when PWR0 goes low
...
static int mxc_epdc_fb_set_par(struct fb_info *info)
{
	...
	*/
	if (!fb_data->hw_ready) {
		...
		/*
		* check the video mode, it must have correct resolutions
		*/
		//if (fb_data->cur_mode != &fb_data->pdata->epdc_mode[i])
		//continue;
...
int mxc_epdc_fb_read_temperature(struct mxc_epdc_fb_data *fb_data)
{
	...
	{
		now = get_seconds();
		if( (now - fb_data->last_time_temp_auto_update) > fb_data->temp_auto_update_period )
		{
...
int mxc_epdc_fb_read_cur_temperature(struct mxc_epdc_fb_data *fb_data)
{
	int temperature=0;
...
int mxc_epdc_fb_set_border_mode(int mode, struct fb_info *info)
{ 
	struct mxc_epdc_fb_data *fb_data = info ?
		(struct mxc_epdc_fb_data *)info:g_fb_data;

	/* Store border mode. Used later when configuring updates. */
	fb_data->border_mode = mode;
	return 0;

}
EXPORT_SYMBOL(mxc_epdc_fb_set_border_mode);
...
struct update_desc_list *update_to_merge)
{
	...
	*/
	...
	if (a->waveform_mode != b->waveform_mode)
		a->waveform_mode = COMBINE_WAVEFORM_MODE(a->waveform_mode, b->waveform_mode);
...
static void epdc_submit_work_func(struct work_struct *work)
{
	...
	*/
	...
	epdc_set_border_gpio(fb_data->border_mode);
						...
						struct fb_info *info)
{
	...
	/*
	printk("<0>send_update %d %d %d %d wf=%d mode=%d\n",
...
int mxc_epdc_fb_wait_update_complete(u32 update_marker, struct fb_info *info)
{
	...
	if (update_marker == 0)
	{
		dev_err(fb_data->dev, "invalid update_marker value 0!\n");
		return -EINVAL;
	}
							 ...
							 unsigned long arg)
{
	...
	case MXCFB_SEND_UPDATE:
		{
			...
			if (!copy_from_user(&upd_data, argp,sizeof(upd_data))) {			
				if(upd_data.update_mode == 1){
					full_updata_mode = true;
				}

				if(screen_updata_flag == false)
					full_updata_ok = false;
				else
					full_updata_ok = true;
				
				screen_updata_flag = false;
				if(FB_Resume_full_updata)
				{
					upd_data.update_mode=UPDATE_MODE_FULL;
					/*wen add */
				        upd_data.waveform_mode=fb_data->wv_modes.mode_gc16;//GC16
				       // upd_data.waveform_mode=WAVEFORM_MODE_AUTO;
				       // upd_data.waveform_mode=fb_data->wv_modes.mode_gl16;
					FB_Resume_full_updata =false;
					full_updata_mode = true;
					full_updata_ok = true;
					//printk("fb UPDATA reflush FULL GC16  !\n"); 
				}
	...
	case MXCFB_UPDATE_WAVEFORM:
		{
			if (copy_to_user(argp, &(g_fb_data->waveform_buffer_phys), sizeof(u32 *))) {
				ret = -EFAULT;
			} else {
				//wf_type = 0x2d;
				ret = 0;
	...
	case MXCFB_GET_PWRDOWN_DELAY:
		{
			...
			temperature = mxc_epdc_fb_read_cur_temperature(fb_data);
	...
	case MXCFB_GET_PMIC_TEMPERATURE:
		{
			...
			temperature = mxc_epdc_fb_read_temperature(fb_data);
			//regulator_enable(fb_data->temp_regulator);
			//temperature = regulator_get_voltage(fb_data->temp_regulator);
			//regulator_disable(fb_data->temp_regulator);
...
static int mxc_epdc_fb_init_hw(struct fb_info *info)
{
	...
	*/
	...
	printk("%s waveformFileName=%s\n",__func__,fb_data->fw_str);
...
//static char *waveform = NULL;
//module_param(waveform, charp, S_IRUGO|S_IWUSR);
...
int __devinit mxc_epdc_fb_probe(struct platform_device *pdev)
{
	...
	if (options){
		...
		//if(panel_str)
		//     printk(KERN_INFO "%s:panel_str=%s\n", __func__, panel_str);		
	}
		...
		fb_data->cur_mode = &fb_data->pdata->epdc_mode[1];
		///fb_data->cur_mode = &fb_data->pdata->epdc_mode[0];///as test
			...
			for (i = 0; i < fb_data->pdata->num_modes; i++){
				if (!strcmp(fb_data->pdata->epdc_mode[i].vmode->name,
					panel_str)) {
						...
						if(strstr(panel_str, "NULL"))
						{
							// disable powerdown delay if NULL waveform
							fb_data->pwrdown_delay = FB_POWERDOWN_DISABLE;
						}
						break;
				}
		...
		*/
		...
		if (IS_ERR(fb_data->tmst_regulator)) {
			dev_err(&pdev->dev, "Unable to get TMST regulator."
...
static __devexit int mxc_epdc_fb_remove(struct platform_device *pdev)
{
	struct update_data_list *plist, *temp_list;
...
static int mxc_epdc_fb_shutdown(struct platform_device *pdev)
{
	...
	if (ret)
	{
		printk("FB_BLANK_POWERDOWN fail !! \n"); 
		goto out;
	}
out:
	/*reflush the screan for white or black for powerdown*/
/*	//suppress clear screen when going power off
	{	
		unsigned long flags;
		int i;
		...
		epdc_set_update_dimensions(data->info.var.xres,
			data->info.var.yres);
		epdc_submit_update(0, data->wv_modes.mode_gc16, UPDATE_MODE_FULL, true, 0xff);//
		//epdc_submit_update(0, data->wv_modes.mode_gl16, UPDATE_MODE_FULL, true, 0xff);//
...
static void mxc_epdc_early_suspend(struct early_suspend *h)
{
	...
	ret = mxc_epdc_fb_blank(FB_BLANK_POWERDOWN, &g_fb_data->info);
	if (ret)
		printk("FB_BLANK_POWERDOWN ERROR! \n");
	...
	if (mxc_epdc_earlysuspend_mode) {
		...
		epdc_set_update_addr(g_fb_data->phys_start);		
		epdc_set_update_coord(0, 0);		
		epdc_set_update_dimensions(g_fb_data->info.var.xres,
					g_fb_data->info.var.yres);
		
		//wendel	epdc_submit_update(0, g_fb_data->wv_modes.mode_gc16, UPDATE_MODE_FULL, true, 0xff);

		//epdc_submit_update(0, g_fb_data->wv_modes.mode_gc16, UPDATE_MODE_FULL, true, 0);
		//epdc_submit_update(0, g_fb_data->wv_modes.mode_gl16, UPDATE_MODE_FULL, true, 0xff);

		for (i = 0; i < 40; i++) {
			if (!epdc_is_lut_active(0)) {
				dev_dbg(g_fb_data->dev, "Mode0 init complete\n");
				break;
			}
			msleep(100);
		}
		g_fb_data->powering_down = true;
		disable_irq(g_fb_data->epdc_irq);		
...
static void mxc_epdc_late_resume(struct early_suspend *h)
{
	...
	if (mxc_epdc_earlysuspend_mode){
		 enable_irq(g_fb_data->epdc_irq);
	         mxc_epdc_earlysuspend_mode =0;		
	}

	mxc_epdc_fb_blank(FB_BLANK_UNBLANK, &g_fb_data->info);

	//FB_Resume_full_updata= true;
}
...
static struct platform_driver mxc_epdc_fb_driver = {
	...
	.remove = __devexit_p(mxc_epdc_fb_remove),
