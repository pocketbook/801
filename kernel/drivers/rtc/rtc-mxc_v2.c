...
static irqreturn_t mxc_rtc_interrupt(int irq, void *dev_id)
{
	...
	u32 lp_status, lp_cr;
	u32 events = 0;
	//printk(KERN_INFO "mxc_rtc_interrupt (1):LPSR=%x\n",  __raw_readl(ioaddr + SRTC_LPSR));
	//printk("bAlarmExist=%d,bHaveAutoPowerDown=%d,bInSuspendState=%d\n",bAlarmExist,bHaveAutoPowerDown,bInSuspendState);
...
static int mxc_rtc_suspend(struct platform_device *pdev, pm_message_t state)
{
	struct rtc_drv_data *pdata = platform_get_drvdata(pdev);
	 int ret =0;
	 int iRTCPowerActionTime =0;
	 int iRTCWakeupSuependTime =0;
