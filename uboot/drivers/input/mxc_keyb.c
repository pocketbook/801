...
extern int mx50_rdp_recovery_button_init(void);
extern int  mx50_rdp_power_button_init(void);
extern int mx50_rdp_vcom_button_init(void);
/*!
...
int my_mxc_kpp_getc(void)
{
	int cnt=0;
	int uData;
#if defined(CONFIG_EPDC_RGB)
#else
	mx50_rdp_recovery_button_init();
	mx50_rdp_power_button_init();

	uData = readl(GPIO6_BASE_ADDR + 0x4); 
	uData &= (~(1<<14));  
	writel(uData, GPIO6_BASE_ADDR + 0x4);
	uData = readl(GPIO6_BASE_ADDR);
	if((uData&(1<<14))==0)
		cnt++; 

	//power key init and count.
	uData = readl(GPIO4_BASE_ADDR + 0x4);
	uData &= (~(1<<15));  
	writel(uData, GPIO4_BASE_ADDR + 0x4); 
	uData = readl(GPIO4_BASE_ADDR);
	if((uData&(1<<15))==0)
		cnt++; 
#endif	
	return cnt;
...
#if defined(BOARD_HAS_HD_SCREEN_1024X758)
	mx50_rdp_back_button_init();

	uData = readl(GPIO2_BASE_ADDR + 0x4);
	uData &= (~(1<<15));  
	writel(uData, GPIO2_BASE_ADDR + 0x4); 
	uData = readl(GPIO2_BASE_ADDR);
	if((uData&(1<<15))==0)
		cnt++; 

#else
	//mx50_rdp_vcom_button_init();
	mx50_rdp_power_button_init();
