...
#define RECOVERY_MODE_VOLUP_POWER 
extern int check_recovery_cmd_file(void);
extern enum boot_device get_boot_device(void);
extern int my_mxc_kpp_getc(void);
...
#if defined(RECOVERY_MODE_DEMO_POWER)
...
#elif defined(RECOVERY_MODE_VOLUP_POWER)
int check_key_pressing(void)
{
	int state = 0;
	//printf("%s %s %d \n",__FILE__,__func__,__LINE__);
	if(my_mxc_kpp_getc()==2)
	{
		state |= PRESSED_HOME;
		state |= PRESSED_POWER;
	}
